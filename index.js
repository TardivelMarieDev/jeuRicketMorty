const clouds = document.querySelectorAll(".hole"); //nuages (type tableau(array))
const scoreBoard = document.querySelector(".score");
const heads = document.querySelectorAll(".head"); //tête
let lastCloud;

let timeUp = false; // false si le jeu n'est pas terminé
let score = 0;


//nombre aleatoire dans une intervalle(temps d'apparition de la tête)
...
function randomCloud(clouds) {
    ...
    if (cloudSelect == lastCloud) { //si le nuage est égal au dernier apparu, relancer la fonction
...
function showHead1() {
    ...
    setTimeout(() => {
        if (!timeUp) showHead1(); //si le jeu n'est pas terminé, montre la tête
        cloud.classList.remove("up");
    }, time); // fonction temps d'apparition de la tête
...
function showHead2() {
    ...
    cloud.classList.add("up");
    setTimeout(() => {
        if (!timeUp) showHead2(); //si le jeu n'est pas terminé, montre la tête
        cloud.classList.remove("up");
    }, time); // fonction temps d'apparition de la tête
...
function showHead3() {
    ...
    cloud.classList.add("up");
    setTimeout(() => {
        if (!timeUp) showHead3(); //si le jeu n'est pas terminé, montre la tête
        cloud.classList.remove("up");
    }, time); // fonction temps d'apparition de la tête
